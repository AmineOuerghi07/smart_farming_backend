...
export class WeatherService {
  ...
  private async formatCityName(city: string): Promise<string> {
    // Si la ville contient déjà :tunisia, la retourner telle quelle
    if (city.includes(':tunisia')) {
      return city;
    }
    
    try {
      // Essayer d'obtenir les coordonnées de la ville
      const url = `${this.geoUrl}/direct?q=${city},tn&limit=1&appid=${this.apiKey}`;
      const response = await axios.get(url);
      
      if (response.data && response.data.length > 0) {
        const { name, country } = response.data[0];
        
        // Vérifier si la ville est en Tunisie (TN)
        if (country === 'TN') {
          return `${name}:tunisia`;
        }
      }
      
      // Si la ville n'est pas en Tunisie, utiliser Tunis
  ...
  async getCityFromCoords(lat: number, lon: number) {
    try {
      ...
      if (lat >= 30.2 && lat <= 37.5 && lon >= 7.5 && lon <= 11.6) {
      ...
      // Si les coordonnées ne sont pas en Tunisie, retourner Tunis
      return 'Tunis:tunisia';
    } catch (error) {
